...
class TestContextManagement:
    ...
    @pytest.mark.asyncio
    async def test_2_2_position_reversal_behavior(self, base_context, mock_order):
        """Test 2.2: Position Reversal Behavior (was: Side Consistency Prevention)"""
        # Setup - create long position first
        base_context["order_manager"].create_and_submit_order.return_value = mock_order
        base_context["order_manager"].create_order.return_value = mock_order
        base_context["order_manager"].cancel_order.return_value = True
        base_context["position_tracker"].get_positions_for_symbol.return_value = []
        base_context["position_tracker"].close_position.return_value = True
        ...
        # Should now succeed by reversing the position
        result = await short_action.execute(base_context)
        assert result is True  # Should succeed (position reversal)
        
        # Should have closed the original position and created new short position
        assert base_context["AAPL"]["side"] == "SELL"
...
class TestPositionConclusion:
    ...
    async def test_4_1_stop_loss_exit_long_position(self, base_context, mock_event_bus):
        ...
        # Verify context was completely cleared
        assert "AAPL" not in base_context
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_7_3_position_creation_after_closure(self, base_context, mock_order):
        """Test 7.3: Position Creation After Previous Position Closure"""
        # Setup - first create and close a position
        base_context["order_manager"].create_and_submit_order.return_value = mock_order
        base_context["order_manager"].cancel_order.return_value = True
        base_context["position_tracker"].get_positions_for_symbol.return_value = []
        base_context["position_tracker"].close_position.return_value = True
        ...
        initial_action = LinkedCreateOrderAction(
            symbol="AAPL",
            quantity=100,
            side="BUY"
        )
        result1 = await initial_action.execute(base_context)
        assert result1 is True
        assert base_context["AAPL"]["side"] == "BUY"
        
        # Close the position completely
        close_action = LinkedCloseAllAction(symbol="AAPL")
        close_result = await close_action.execute(base_context)
        assert close_result is True
        
        # Context should be completely cleared
        assert "AAPL" not in base_context
        
        # Create new SELL position - should work cleanly
        new_action = LinkedCreateOrderAction(
            symbol="AAPL",
            quantity=100,
            side="SELL"
        )
        result2 = await new_action.execute(base_context)
        
        # Should succeed with fresh context
...
# ===== 8. POSITION REVERSAL LOGIC =====

class TestPositionReversal:
    """Test 8: Position reversal logic - one trade per symbol."""
    
    @pytest.mark.asyncio
    async def test_8_1_same_side_signal_ignored(self, base_context, mock_order):
        """Test 8.1: Same Side Signal Ignored"""
        # Setup existing BUY position
        base_context["order_manager"].create_and_submit_order.return_value = mock_order
        base_context["order_manager"].create_order.return_value = mock_order
        base_context["AAPL"] = {
            "side": "BUY",
        ...
        # Create another BUY action (same side)
        action = LinkedCreateOrderAction(
            symbol="AAPL",
            quantity=100,
            side="BUY",
        ...
        # Should be ignored (return True but no orders created)
        assert result is True
        base_context["order_manager"].create_and_submit_order.assert_not_called()
        base_context["order_manager"].create_order.assert_not_called()
        
        # Context should remain unchanged
        assert base_context["AAPL"]["side"] == "BUY"
        assert base_context["AAPL"]["main_orders"] == ["ORDER123"]
    
    @pytest.mark.asyncio
    async def test_8_2_opposite_side_reversal(self, base_context, mock_order):
        """Test 8.2: Opposite Side Position Reversal"""
        # Setup existing BUY position
        from unittest.mock import AsyncMock
        
        base_context["order_manager"].create_and_submit_order.return_value = mock_order
        base_context["order_manager"].create_order.return_value = mock_order
        base_context["order_manager"].cancel_order = AsyncMock()
        base_context["position_tracker"].get_positions_for_symbol = AsyncMock(return_value=[])
        base_context["position_tracker"].close_position = AsyncMock()
        
        # Setup existing BUY position in context
        base_context["AAPL"] = {
            "side": "BUY",
            "main_orders": ["ORDER123"],
            "stop_orders": ["STOP123"],
        ...
        # Create SELL action (opposite side)
        action = LinkedCreateOrderAction(
            symbol="AAPL",
            quantity=100,
            side="SELL",
        ...
        # Should succeed after closing existing position
        assert result is True
        
        # Should have canceled existing orders (called by LinkedCloseAllAction)
        assert base_context["order_manager"].cancel_order.call_count >= 3  # main, stop, target
        
        # Should have created new SELL order
        base_context["order_manager"].create_and_submit_order.assert_called()
        call_args = base_context["order_manager"].create_and_submit_order.call_args
        assert call_args[1]["quantity"] == -100  # Negative for SELL
        
        # Context should be updated to SELL
        assert base_context["AAPL"]["side"] == "SELL"
        assert base_context["AAPL"]["status"] == "active"
    
    @pytest.mark.asyncio
    async def test_8_3_no_position_normal_entry(self, base_context, mock_order):
        """Test 8.3: No Existing Position - Normal Entry"""
        # Setup clean context (no existing position)
        base_context["order_manager"].create_and_submit_order.return_value = mock_order
        base_context["order_manager"].create_order.return_value = mock_order
        
        # Create action with no existing position
        action = LinkedCreateOrderAction(
            symbol="AAPL",
            quantity=100,
            side="BUY",
