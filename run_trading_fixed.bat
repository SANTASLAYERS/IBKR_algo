@echo off
echo ========================================
echo Alpaca Trading System Launcher (FIXED)
echo ========================================
echo.
echo FIXES APPLIED:
echo - ATR calculation now uses 1-minute bars
echo - Event loop concurrency issues resolved
echo - Dynamic position sizing working
echo.
...
echo Starting trading system with fixes...
echo.
echo Alpaca Mode: %ALPACA_TRADING_MODE%
echo API Endpoint: %API_BASE_URL%
echo.
echo NOTE: ATR calculations will now use 1-minute bars
echo       (Alpaca does not support sub-minute timeframes)
echo.
